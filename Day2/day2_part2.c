...
long long pseudo_invalid_id(char *id_str) {
    // trying to pseudo code the function first
    ...
    for (long long current_id = id_start; current_id <= id_end; current_id++) {
        ...
        for (int pattern_length = 1; pattern_length <= id_length / 2; pattern_length++) {
            if (id_length % pattern_length == 0) {
                char pattern[pattern_length + 1];
                strncpy(pattern, id, pattern_length);
                pattern[pattern_length] = '\0';

                int is_repeated = 1;

                for (int pos = pattern_length; pos < id_length; pos += pattern_length) {
                    if (strncmp(id + pos, pattern, pattern_length) != 0) {
                        is_repeated = 0;
                        break;
                    }
                }
                if (is_repeated) {
                    sum += current_id;
                    break;  // No need to check further pattern lengths
                }
            }
        }
    }
...
long long split_buffer_to_ids(char *buffer) {
    ...
    while (token != NULL) {
        ...
        long long result = pseudo_invalid_id(token_copy);
